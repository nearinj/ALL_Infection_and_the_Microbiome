...
make_boxplots <- function(plot_data, diff_taxa, index){
  plot <- ggplot(plot_data, aes(Pre_Post_never_infect, diff_taxa[,index], fill=plot_data$State_Cols))+ geom_boxplot(outlier.shape = NA) +
...
post_never_species <- get_diff_taxa(metaphlan_spec, metadata_post_never)
colnames(post_never_species)
post_never_plot <- get_boxplots(post_never_species, metadata_post_never, "s")
post_never_plot[[2]]
Sup_Fig_13 <- plot_grid(plotlist=c(pre_never_order_plot, post_never_plot))
Sup_Fig_13
